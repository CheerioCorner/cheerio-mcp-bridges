 ...
 *
 * Bridges spawn CLIs pinned to a workspace directory that is inherently
 * machine/user-specific (someone's local checkout). Baking any one
 * person's path in as a silent fallback means a teammate's misconfigured
 * (or unconfigured) server quietly runs against the wrong — or a
 * nonexistent — directory. Failing at startup instead surfaces the
 * problem immediately, in the MCP connection status, instead of letting
 * it masquerade as a working server that answers from the wrong place.
 *
...
export function requireEnv(name) {
  ...
  if (!v) {
    throw new Error(
      `${name} is not set. This bridge has no built-in default workspace directory — ` +
        `set ${name} in this MCP server's "env" block (see .mcp.json) to the absolute ` +
        `path of your local workspace root.`
    );
  }
  return v;
}
